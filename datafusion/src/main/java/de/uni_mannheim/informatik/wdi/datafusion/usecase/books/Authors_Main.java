...
//		ds4.setScore(2.0);
		...
		ds1.setDate(DateTime.parse("2015-12-05"));
...
//		ds4.printDataSetDensityReport();
		...
		correspondences.loadCorrespondences(new File("usecase/books/correspondences/FusedBooks_Authors_correspondences.csv"), ds1, ds2);
...
//		
		...
		// write the result
		fusedDataSet.writeXML(new File("usecase/books/output/BooksAndAuthorsfused.xml"), new BookXMLFormatter());
		
		// load the gold standard
		DataSet<FusableBooks> gs = new FusableDataSet<>();
		gs.loadFromXML(
				new File("usecase/books/goldstandard/GS_Fusion_Books_Authors.xml"),
				new FusableBooksFactory(), "/Books/Book");
